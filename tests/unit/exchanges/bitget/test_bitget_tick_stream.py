...
@pytest.mark.asyncio
async def test_j27_h19_h20_brief_flap_tip_advances_write_liveness_pass(
    mocker, mock_ws_manager, mock_parser, caplog, tmp_path
):
    """J27 H19/H20 healthy public flap: brief reconnect + tip resume + write PASS.

    Chronology (beta 2026-08-07 H19 ~1.38s / H20 ~1.25s):
      opaque WS close → reconnect → sub XRPUSDT confirmed → write-liveness armed
      → tip advances / journal write within grace. Must NOT dual-heal (contrast
      J21 tip-freeze). Write-liveness must remain armed on post-flap confirm.
    """
    import logging
    ...
    tip = {"seq": 2_122_240}  # J27 H19 tip at flap
    ...
    # Long-lived collector: boot confirm satisfied (mirrors gen before H19 flap).
    await captured[0]()
    assert stream.connect_generation == 1
    guard.arm_after_subscriptions_confirmed({"XRPUSDT"}, connect_generation=1)
    guard.record_journal_write()
    ...
    async def mock_listen():
        yield orjson_subscribe_ack("XRPUSDT")
        # Satisfy write-liveness via journal write (post-flap recovery).
        guard.record_journal_write()
        tip["seq"] = 2_123_328
    ...
    with caplog.at_level(logging.INFO):
        mock_ws_manager.flap_tip_monitor.note_connection_closed(
            close_wall_ms=19_19_36_876, close_mono_ms=100
        )
        mock_ws_manager.flap_tip_monitor.note_connection_restored(
            reconnect_wall_ms=19_19_38_257, reconnect_mono_ms=1_484
        )
        await captured[0]()
        assert stream.connect_generation == 2
        await stream.start_streaming()
    ...
    assert any(
        ...
        and "since_close_ms=1384" in r.message
        and "tip_seq_before=2122240" in r.message
        for r in caplog.records
    )
    assert any(
...
@pytest.mark.asyncio
async def test_j28_h20_h21_brief_flap_tip_advances_write_liveness_pass(
    mocker, mock_ws_manager, mock_parser, caplog, tmp_path
):
    """J28 H20/H21 healthy public flap: brief reconnect + tip resume + write PASS.

    Chronology (beta 2026-08-08 H20 ~1.249s / H21 ~1.25s):
      opaque WS close → reconnect → sub XRPUSDT confirmed → write-liveness armed
      (arm_count 7→8) → tip advances / journal write within grace. Must NOT
      dual-heal. Contrast J28 H19: private-only flap does not use this path.
    """
    import logging
    ...
    tip = {"seq": 2_171_321}  # J28 H20 tip at flap
    write_stale: list[object] = []
    flap_stale: list[object] = []
    ...
    # Long-lived collector: prior confirms satisfied (mirrors gen before H20 flap).
    await captured[0]()
    assert stream.connect_generation == 1
    ...
    with caplog.at_level(logging.INFO):
        mock_ws_manager.flap_tip_monitor.note_connection_closed(
            close_wall_ms=20_24_09_199, close_mono_ms=100
        )
        mock_ws_manager.flap_tip_monitor.note_connection_restored(
            reconnect_wall_ms=20_24_10_448, reconnect_mono_ms=1_349
        )
        await captured[0]()
        assert stream.connect_generation == 2
        await stream.start_streaming()
    ...
    assert any(
        "post_flap_correlation" in r.message
        and "brief_public_flap=True" in r.message
        and "since_close_ms=1249" in r.message
        and "tip_seq_before=2171321" in r.message
        for r in caplog.records
    )
    assert any(
        "Post-confirm write-liveness armée" in r.message
...
@pytest.mark.asyncio
async def test_j31_h00_h21_brief_flap_tip_advances_write_liveness_pass(
    mocker, mock_ws_manager, mock_parser, caplog, tmp_path
):
    """J31 H00/H21 healthy public flap: brief reconnect + tip resume + write PASS.

    Chronology (beta 2026-08-11):
      H00 pid=613960 ~1.254s tip 2316441 arm_count=2 (v0.18.33)
      H21 pid=615212 ~1.244s tip 2405725 arm_count=2 gen=2 (v0.18.35)
      opaque WS close → reconnect → sub XRPUSDT confirmed → write-liveness armed
      → tip advances / journal write within grace. Must NOT dual-heal
      (BB-B5-A1 KNOWN_EXPECTED). Contrast H21 private-only @21:49.
    """
    import logging
    ...
    tip = {"seq": 2_316_441}  # J31 H00 tip at flap
    write_stale: list[object] = []
    flap_stale: list[object] = []
    ...
    # Boot confirm satisfied (mirrors gen before H00/H21 flap → arm_count=2).
    await captured[0]()
    assert stream.connect_generation == 1
    guard.arm_after_subscriptions_confirmed({"XRPUSDT"}, connect_generation=1)
    guard.record_journal_write()
    ...
    with caplog.at_level(logging.INFO):
        mock_ws_manager.flap_tip_monitor.note_connection_closed(
            close_wall_ms=46_26_018, close_mono_ms=100
        )
        mock_ws_manager.flap_tip_monitor.note_connection_restored(
            reconnect_wall_ms=46_27_272, reconnect_mono_ms=1_354
        )
        await captured[0]()
        assert stream.connect_generation == 2
        await stream.start_streaming()
    ...
    assert any(
        "post_flap_correlation" in r.message
        and "brief_public_flap=True" in r.message
        and "since_close_ms=1254" in r.message
        and "tip_seq_before=2316441" in r.message
        for r in caplog.records
    )
    assert any(
        "Post-confirm write-liveness armée" in r.message
