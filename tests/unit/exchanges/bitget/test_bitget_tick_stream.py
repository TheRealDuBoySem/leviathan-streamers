...
@pytest.mark.asyncio
async def test_resubscribe_empty(mocker, stream):
    mock_send = mocker.patch('core.network.reconnecting_ws_manager.ReconnectingWebSocketManager.send', new_callable=AsyncMock)
...
async def test_delegated_methods(mocker, stream):
    mock_get = mocker.patch('core.routing.async_queue_dispatcher.AsyncQueueDispatcher.wait_for_next_data', new_callable=AsyncMock)
    await stream.wait_for_next_tick()
    assert mock_get.call_count == 1
    
    mock_task_done = mocker.patch('core.routing.async_queue_dispatcher.AsyncQueueDispatcher.task_done')
    stream.mark_tick_as_processed()
    assert mock_task_done.call_count == 1
...
async def test_bitget_tick_stream_aiter_exception(mocker, mock_ws_manager, mock_parser):
    ...
    mocker.patch.object(dispatcher, 'wait_for_next_data', side_effect=RuntimeError("test error"))
...
async def test_base_exchange_stream_observers(mocker, stream):
    """Test the Observer pattern in BaseExchangeStream."""
    ...
    await stream._notify_observers(tick)
...
async def test_start_streaming_logic(mocker, stream):
    ...
    mocker.patch.object(stream._network_manager, 'start_connection_and_listen', side_effect=mock_listen)
    
    def mock_parse(msg):
        if msg == "sys": return SystemMessage(event="info", msg="sys_msg")
        if msg == "err": return ErrorMessage(msg="err_msg")
        return None
        
    mocker.patch.object(stream._parsing_strategy, 'parse', side_effect=mock_parse)
