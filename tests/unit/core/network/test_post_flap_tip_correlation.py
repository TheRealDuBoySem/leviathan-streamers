...
@pytest.mark.asyncio
async def test_j28_h20_brief_1249ms_flap_tip_advances_no_stale(caplog):
    """J28 H20 @20:24: opaque close → reconnect ~1.249s → tip 2171321 resumes.

    Evidence: brief_public_flap=True, tip frozen only during the flap hole,
    write-liveness PASS later — no post_flap_tip_stale.
    """
    tip = {"seq": 2_171_321}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        monitor.note_connection_closed(close_wall_ms=20_24_09_199, close_mono_ms=100)
        monitor.note_connection_restored(
            reconnect_wall_ms=20_24_10_448,
            reconnect_mono_ms=1_349,  # +1249 ms (J28 H20)
        )

    assert monitor.last_flap_duration_ms == 1249
    assert monitor.is_awaiting_tip_progress() is True
    correlation = next(r.message for r in caplog.records if "post_flap_correlation" in r.message)
    assert "since_close_ms=1249" in correlation
    assert "brief_public_flap=True" in correlation
    assert "tip_seq_before=2171321" in correlation
    assert "tip_seq_after=2171321" in correlation

    # Tip resumes after flap (H20 tip later 2171828 before soft-stale).
    tip["seq"] = 2_171_828
    assert monitor.record_tip_progress() is True
    assert monitor.is_awaiting_tip_progress() is False
...
@pytest.mark.asyncio
async def test_j28_h21_brief_1250ms_flap_tip_advances_no_stale(caplog):
    """J28 H21 @21:20: opaque close → reconnect ~1.25s → tip 2172138 resumes.

    Complements H20: same healthy brief-public-flap signature; tip advances
    before write-liveness timeout (burst @21:25) — no tip-stale heal.
    """
    tip = {"seq": 2_172_138}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        monitor.note_connection_closed(close_wall_ms=21_20_47_483, close_mono_ms=100)
        monitor.note_connection_restored(
            reconnect_wall_ms=21_20_48_730,
            reconnect_mono_ms=1_350,  # +1250 ms (J28 H21)
        )
    ...
    assert "since_close_ms=1250" in correlation
    assert "brief_public_flap=True" in correlation
    assert "tip_seq_before=2172138" in correlation
    assert "tip_seq_after=2172138" in correlation

    tip["seq"] = 2_172_879  # tip advances by end of H21
    assert monitor.record_tip_progress() is True
...
@pytest.mark.asyncio
async def test_j28_h19_private_only_flap_does_not_arm_public_tip_correlation(caplog):
    """J28 H19: private WS flap ~374 ms; public collector flap ABSENT vs J27 H19.

    SetupExecutor private lost→restored must not be treated as a public collector
    flap: without ``note_connection_closed``, restore is a no-op and tip-stale
    stays unarmed (no false ``post_flap_correlation``).
    """
    tip = {"seq": 2_169_478}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        # Private flap duration only (~374 ms) — no public close noted.
        monitor.note_connection_restored(
            reconnect_wall_ms=19_10_29_489,
            reconnect_mono_ms=474,
        )

    assert monitor.last_flap_duration_ms is None
    assert monitor.is_awaiting_tip_progress() is False
    assert not any("post_flap_correlation" in r.message for r in caplog.records)

    tip["seq"] = 2_170_628  # tip still advances during H19 without public flap
...
@pytest.mark.asyncio
async def test_j29_h00_brief_1241ms_flap_tip_advances_no_stale(caplog):
    """J29 H00 @00:39: opaque close → reconnect ~1.241s → tip 2176192 resumes.

    Evidence: brief_public_flap=True, tip frozen only during the flap hole,
    write-liveness armée arm_count=9 — no post_flap_tip_stale (BB-B5-A1 healthy).
    """
    tip = {"seq": 2_176_192}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        monitor.note_connection_closed(close_wall_ms=39_42_006, close_mono_ms=100)
        monitor.note_connection_restored(
            reconnect_wall_ms=39_43_248,
            reconnect_mono_ms=1_341,  # +1241 ms (J29 H00)
        )

    assert monitor.last_flap_duration_ms == 1241
    assert monitor.is_awaiting_tip_progress() is True
    correlation = next(r.message for r in caplog.records if "post_flap_correlation" in r.message)
    assert "since_close_ms=1241" in correlation
    assert "brief_public_flap=True" in correlation
    assert "tip_seq_before=2176192" in correlation
    assert "tip_seq_after=2176192" in correlation

    tip["seq"] = 2_176_628  # tip resumes post-flap (H00 tip ~2176628+)
    assert monitor.record_tip_progress() is True
    assert monitor.is_awaiting_tip_progress() is False
...
@pytest.mark.asyncio
async def test_j29_h21_brief_1262ms_flap_tip_advances_no_stale(caplog):
    """J29 H21 @21:46: opaque close → reconnect ~1.262s → tip 2211489 resumes.

    Complements H00: same healthy brief-public-flap signature; write-liveness
    arm_count=2 connect_generation=2 PASS — no tip-stale heal.
    """
    tip = {"seq": 2_211_489}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        monitor.note_connection_closed(close_wall_ms=21_46_50_067, close_mono_ms=100)
        monitor.note_connection_restored(
            reconnect_wall_ms=21_46_51_329,
            reconnect_mono_ms=1_362,  # +1262 ms (J29 H21)
        )

    assert monitor.last_flap_duration_ms == 1262
    correlation = next(r.message for r in caplog.records if "post_flap_correlation" in r.message)
    assert "since_close_ms=1262" in correlation
    assert "brief_public_flap=True" in correlation
    assert "tip_seq_before=2211489" in correlation
    assert "tip_seq_after=2211489" in correlation

    tip["seq"] = 2_211_715  # tip advances by end of H21
    assert monitor.record_tip_progress() is True
...
@pytest.mark.asyncio
async def test_j29_h21_private_only_flap_does_not_arm_public_tip_correlation(caplog):
    """J29 H21 @21:12: private WS flap ~375 ms must not arm public tip correlation.

    SetupExecutor private lost→restored (~375 ms) is independent of the later
    public collector flap @21:46; without ``note_connection_closed``, restore
    is a no-op (no false ``post_flap_correlation`` / tip-stale).
    """
    tip = {"seq": 2_210_415}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        # Private flap duration only (~375 ms) — no public close noted.
        monitor.note_connection_restored(
            reconnect_wall_ms=21_12_51_738,
            reconnect_mono_ms=475,
        )
    ...
    tip["seq"] = 2_210_615  # tip still advances after private restore (burst)
    assert monitor.record_tip_progress() is False
...
@pytest.mark.asyncio
async def test_j31_h00_brief_1254ms_flap_tip_advances_no_stale(caplog):
    """J31 H00 @00:46: opaque close → reconnect ~1.254s → tip 2316441 resumes.

    Evidence (collector pid=613960 / v0.18.33): brief_public_flap=True, tip frozen
    only during the flap hole, write-liveness armée arm_count=2 — no
    post_flap_tip_stale (BB-B5-A1 KNOWN_EXPECTED healthy recovery).
    """
    tip = {"seq": 2_316_441}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        monitor.note_connection_closed(close_wall_ms=46_26_018, close_mono_ms=100)
        monitor.note_connection_restored(
            reconnect_wall_ms=46_27_272,
            reconnect_mono_ms=1_354,  # +1254 ms (J31 H00)
        )

    assert monitor.last_flap_duration_ms == 1254
    assert monitor.is_awaiting_tip_progress() is True
    correlation = next(r.message for r in caplog.records if "post_flap_correlation" in r.message)
    assert "since_close_ms=1254" in correlation
    assert "brief_public_flap=True" in correlation
    assert "tip_seq_before=2316441" in correlation
    assert "tip_seq_after=2316441" in correlation

    tip["seq"] = 2_316_576  # tip resumes post-flap (H00 soft-stale tip 2316576+)
    assert monitor.record_tip_progress() is True
    assert monitor.is_awaiting_tip_progress() is False
...
@pytest.mark.asyncio
async def test_j31_h21_brief_1244ms_flap_tip_advances_no_stale(caplog):
    """J31 H21 @21:13: opaque close → reconnect ~1.244s → tip 2405725 resumes.

    Evidence (collector pid=615212 / v0.18.35): brief_public_flap=True, tip frozen
    only during the flap hole, write-liveness arm_count=2 connect_generation=2 —
    no post_flap_tip_stale (BB-B5-A1 KNOWN_EXPECTED healthy recovery).
    """
    tip = {"seq": 2_405_725}
    stale_calls: list[object] = []
    monitor = PostFlapTipCorrelationMonitor(
    ...
    with caplog.at_level(logging.INFO):
        monitor.note_connection_closed(close_wall_ms=21_13_44_897, close_mono_ms=100)
        monitor.note_connection_restored(
            reconnect_wall_ms=21_13_46_140,
            reconnect_mono_ms=1_344,  # +1244 ms (J31 H21)
        )

    assert monitor.last_flap_duration_ms == 1244
    correlation = next(r.message for r in caplog.records if "post_flap_correlation" in r.message)
    assert "since_close_ms=1244" in correlation
    assert "brief_public_flap=True" in correlation
    assert "tip_seq_before=2405725" in correlation
    assert "tip_seq_after=2405725" in correlation

    tip["seq"] = 2_405_800  # tip advances with engine HB ticks post-flap
    assert monitor.record_tip_progress() is True
...
@pytest.mark.asyncio
async def test_j31_h21_private_only_flap_does_not_arm_public_tip_correlation(caplog):
    """J31 H21 @21:49: private WS flap ~367 ms must not arm public tip correlation.

    SetupExecutor private lost→restored (~367 ms) is independent of the earlier
    public collector flap @21:13; without ``note_connection_closed``, restore
    is a no-op (no false ``post_flap_correlation`` / tip-stale).
    """
    tip = {"seq": 2_406_000}
    stale_calls: list[object] = []
    ...
    with caplog.at_level(logging.INFO):
        # Private flap duration only (~367 ms) — no public close noted.
        monitor.note_connection_restored(
            reconnect_wall_ms=21_49_32_868,
            reconnect_mono_ms=467,
        )
