...
class TickJournalMetaStore:
    ...
    def __init__(self, meta_path: str, *, dedup_window: int) -> None:
        ...
        # BB-D23-02 / J25–J29: coherent tip high-water — never report a
        ...
        # multi-h H04/H08/H10–11/H13–14/H16/H19/H23).
    ...
    def read_latest_seq_from_disk(self) -> int:
        ...
        BB-D23-02 / J25–J29: rejects phantom tip rewinds below the observed
        ...
        H23~2216660). High-water advances only on successful disk
    ...
    def reload_from_disk(self) -> None:
        ...
        BB-D23-02 / J25–J29: ``latest_seq`` is clamped to the coherent
        high-water so a phantom disk rewind (e.g. ``1810648`` after ~2.03M
        J27 H02 / ~2.13M H23 / J28 H03~2.138M / H22~2.173M / J29 H02
        ~2.179M; or J29 post-respawn ``2182116`` after H04~2.182M /
        H23~2.216M) cannot poison ``latest_seq()`` / soft-stale
